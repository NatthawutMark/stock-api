...
export const reportSummary = (cwd, packResult, config, options = {}) => {
    ...
    if (config.skillGenerate !== undefined && options.skillDir) {
        ...
        if (packResult.outputFiles && packResult.outputFiles.length > 0) {
            ...
            logger.log(`       Output: ${firstDisplayPath} ${pc.dim('…')} ${lastDisplayPath} ${pc.dim(`(${packResult.outputFiles.length} parts)`)}`);
        }
        else {
...
export const reportTopFiles = (fileCharCounts, fileTokenCounts, topFilesLength, totalTokens) => {
    ...
    filesWithTokenCounts.forEach(([filePath, tokenCount], index) => {
        ...
        const indexString = `${index + 1}.`.padEnd(3, ' ');
        logger.log(`${indexString} ${filePath} ${pc.dim(`(${tokenCount.toLocaleString()} tokens, ${charCount.toLocaleString()} chars, ${percentageOfTotal}%)`)}`);
    });
};
