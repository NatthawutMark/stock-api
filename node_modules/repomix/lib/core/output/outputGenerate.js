...
import { generateTreeString, generateTreeStringWithRoots } from '../file/fileTreeGenerate.js';
...
const calculateMarkdownDelimiter = (contents) => {
...
const generateParsableXmlOutput = async (renderContext) => {
...
const generateParsableJsonOutput = async (renderContext) => {
    const jsonDocument = {
...
const generateHandlebarOutput = async (config, renderContext, processedFiles) => {
    try {
...
export const generateOutput = async (rootDirs, config, processedFiles, allFilePaths, gitDiffResult = undefined, gitLogResult = undefined, filePathsByRoot, emptyDirPaths, deps = {
    ...
    const outputGeneratorContext = await deps.buildOutputGeneratorContext(rootDirs, config, allFilePaths, sortedProcessedFiles, gitDiffResult, gitLogResult, filePathsByRoot, emptyDirPaths);
...
export const buildOutputGeneratorContext = async (rootDirs, config, allFilePaths, processedFiles, gitDiffResult = undefined, gitLogResult = undefined, filePathsByRoot, emptyDirPaths, deps = {
    listDirectories,
    listFiles,
    searchFiles,
}) => {
    ...
    const toOutputDisplayPath = (rootDir, filePath, index) => buildFileDisplayPath({
        rootDir,
        filePath,
        cwd: config.cwd,
    ...
    if (shouldUseFullTree) {
        try {
            ...
            const allDirectories = Array.from(new Set(allDirectoriesByRoot.flatMap((directories, index) => {
                const rootDir = rootDirs[index];
                if (!rootDir)
                    return [];
                return directories.map((directoryPath) => toOutputDisplayPath(rootDir, directoryPath, index));
            }))).sort();
            const allRepoFiles = Array.from(new Set(allFilesByRoot.flatMap((files, index) => {
                const rootDir = rootDirs[index];
                if (!rootDir)
                    return [];
